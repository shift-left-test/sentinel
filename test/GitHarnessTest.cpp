...
class GitHarnessTest : public ::testing::Test {
 protected:
  void SetUp() override {
    repo_name = testTempDir("SENTINEL_GITHARNESSTEST_TMP_DIR");
...
TEST_F(GitHarnessTest, testInitFailWhenDirAlreadyExists) {
  EXPECT_THROW(std::unique_ptr<GitHarness> test_repo = std::make_unique<GitHarness>(repo_name), std::runtime_error);
}
...
TEST_F(GitHarnessTest, testAddFileWorks) {
  ...
  std::string inserted_content((std::istreambuf_iterator<char>(created_file)), (std::istreambuf_iterator<char>()));
  created_file.close();
...
TEST_F(GitHarnessTest, testAddCodeWorks) {
  ...
  std::string new_content((std::istreambuf_iterator<char>(nonempty_file)), (std::istreambuf_iterator<char>()));
  nonempty_file.close();
...
TEST_F(GitHarnessTest, testAddCodeFailWhenPositionOutOfBound) {
  ...
  std::string new_content((std::istreambuf_iterator<char>(file)), (std::istreambuf_iterator<char>()));
  file.close();
  EXPECT_EQ("int main() {}\n//comment\n//add\n//add\n", new_content);
}
...
TEST_F(GitHarnessTest, testStageFileWorks) {
  ...
  GitHarness::libgitErrorCheck(git_status_list_new(&status, repo->getGitRepo(), &statusopt), "Could not get status");
...
TEST_F(GitHarnessTest, testAddTagLightweightFailWhenCommitAlreadyTagged) {
  ...
  repo->addFile("temp2.cpp", initial_content).stageFile(target_files).commit(message);
