...
bool ROR::canMutate(
  ...
  return relational_operators.find(binary_operator) != relational_operators.end();
...
void ROR::mutate(clang::Stmt *s, MutantDatabase *database) {
  BinaryOperator *bo;
...
void ROR::mutateOperator(
  ...
  for (auto mutated_token: relational_operators)
