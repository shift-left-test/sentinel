...
CommandEvaluate::CommandEvaluate(args::Subparser& parser) :
    Command(parser),
    mMutantStr(parser, "mutant", "Mutant string", {'m', "mutant"}, args::Options::Required),
    mExpectedDir(parser, "PATH", "Expected result directory", {'e', "expected"}, args::Options::Required),
    mActualDir(parser, "PATH", "Actual result directory", {'a', "actual"}, args::Options::Required),
    mEvalFile(parser, "FILENAME", "Evaluated output file name which will be joined with output-dir.",
...
int CommandEvaluate::run() {
  ...
  if (mTestState.Get() != "build_failure" && mTestState.Get() != "timeout" && mTestState.Get() != "success" &&
