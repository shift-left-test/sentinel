...
#   include <cstdarg>
  ...
  static inline int TIXML_SNPRINTF(char* buffer, size_t size,
                                   const char* format, ...) {
    va_list va;
    va_start(va, format);
  ...
  static inline int TIXML_VSNPRINTF(char* buffer, size_t size,
  ...
  #define TIXML_SSCANF  sscanf
    ...
    #define TIXML_VSCPRINTF   _vscprintf
    // VS2003's C runtime has this,
    // but VC6 C runtime or WinCE SDK doesn't have.
  #else
    // Microsoft Visual Studio 2003 and earlier or WinCE.
...
char* StrPair::ParseText(char* p, const char* endTag,
...
void XMLUtil::ConvertUTF32ToUTF8(uint32_t input, char* output,
                                  int* length) {
  const uint32_t BYTE_MASK = 0xBF;
  const uint32_t BYTE_MARK = 0x80;
  const uint32_t FIRST_BYTE_MARK[7] = { 0x00, 0x00, 0xC0, 0xE0, 0xF0,
                                             0xF8, 0xFC };
  if (input < 0x80) {
    *length = 1;
  } else if (input < 0x800) {
...
XMLNode* XMLNode::DeepClone(XMLDocument* target) const {
  ...
  if (!clone) return 0;
  for (const XMLNode* child = this->FirstChild();
       child;
...
XMLDocument::XMLDocument(bool processEntities, Whitespace whitespaceMode) :
  ...
  _textPool(),
  _commentPool() {
  // avoid VC++ C4355 warning about 'this' in initializer list
  // (C4355 is off by default in VS2012+)
  _document = this;
}
...
void XMLDocument::DeepCopy(XMLDocument* target) const {
  ...
  target->Clear();
  for (const XMLNode* node = this->FirstChild();
       node;
...
XMLError XMLDocument::LoadFile(FILE* fp) {
  ...
  uint64_t filelength;
  {
    const int64_t fileLengthSigned = TIXML_FTELL(fp);
    TIXML_FSEEK(fp, 0, SEEK_SET);
    ...
    filelength = static_cast<uint64_t>(fileLengthSigned);
  }

  const size_t maxSizeT = static_cast<size_t>(-1);
  // We'll do the comparison as an uint32_t long,
  // because that's guaranteed to be at
  // least 8 bytes, even on a 32-bit platform.
  if (filelength >= static_cast<uint64_t>(maxSizeT)) {
    // Cannot handle files which
    // won't fit in buffer together with null terminator
    SetError(XML_ERROR_FILE_READ_ERROR, 0, 0);
    return _errorID;
  }
...
void XMLDocument::SetError(XMLError error, int lineNum,
                           const char* format, ...) {
      ...
      if (*q > 0 && *q < ENTITY_RANGE) {
        ...
        if (flag[static_cast<unsigned char>(*q)]) {
          ...
          if (!entityPatternPrinted) {
            // TIXMLASSERT(entityPatternPrinted)
            // causes gcc -Wunused-but-set-variable in release
            TIXMLASSERT(false);
          }
          ++p;
        }
...
bool XMLPrinter::VisitEnter(const XMLElement& element,
  ...
  const bool compactMode = parentElem
